...
that takes a function and iterable as inputs and 
returns an iterator with the elements from the 
iterable for which the function returns True.
 The code below uses `filter()` to get the names in `cities` 
 that are fewer than 10 characters long 
...
shor_cty = list(filter(lambda x: len(x) < 10 ,cities))
print(shor_cty)
